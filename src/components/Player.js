import { Body, Bodies, Events, Vertices } from "matter-js";
  ...
  constructor(body, isOnGround) {
    ...
    this.grab = function (obj, initialDistance) {
      var divider = 2,
        multiplier =
          Math.sin(
            [0.0, this.getDistanceFrom(obj), initialDistance].scaleBetween(
              0,
              Math.PI / divider
            )[1]
          ) * 0.1,
        polarDirection = Math.atan2(
          this.position.y - 75 - obj.position.y,
    ...
    this.throw = function (obj, to) {
      ...
      var speed = 1.01,
        polarDirection = Math.atan2(
          to.position.y - obj.position.y,
          to.position.x - obj.position.x
        ),
    ...
    this.move = function (keys, mouseConstraint) {
      var speedCap = 10,
        ...
        speed = {
          x:
            keys["68"] || keys["65"]
              ? Math.abs(this.velocity.x) > speedCap
                ? stopped
                : baseRunningSpeed
              : stopped,
          y:
            keys["32"] || keys["87"]
              ? Math.abs(this.velocity.y) > speedCap
                ? stopped
                : jumpForce
              : stopped,
        };
      var texturePath = "/img/player/";
      if (!this.isOnGround) texturePath += "jumping";
      else texturePath += "standing";
      if (this.position.x <= mouseConstraint.mouse.position.x)
        texturePath += ".right.png";
      else texturePath += ".left.png";
